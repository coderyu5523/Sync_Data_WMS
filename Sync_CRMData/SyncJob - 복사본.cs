    ...
    public class SyncJob : IJob
    {
        ...
        public async Task Execute(IJobExecutionContext context)
        {
            try
            {
                ...
                SqlLogger sqlLogger = new SqlLogger(dbConnectionInfoProvider);
                if (SyncDirection.StartsWith("Bi"))
                {
                    selectedProcessor = new DataSyncLogProcessor(sqlLogger, dbConnectionInfoProvider, syncTaskJob);
                    //selectedProcessor = new DataSyncLogProcessorForUpdate(_logger,dbConnectionInfoProvider, syncTaskJob);
                }
                else 
                {
                    selectedProcessor = new DataSyncLogProcessorForTransfer(sqlLogger, dbConnectionInfoProvider, syncTaskJob);
                }

                if (selectedProcessor != null)
                {
                    // SyncManager에 선택된 프로세서 주입 후 실행
                    SyncManager syncManager = new SyncManager(selectedProcessor, sqlLogger);
