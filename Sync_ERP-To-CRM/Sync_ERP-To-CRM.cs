...
namespace DioImplant_batch
{
    public partial class MainForm : Form
    {
        ...
        private readonly DataSyncLog_OrderUpdate _dataSyncOrderUpdate;
        ...
        public MainForm(DataSyncLog_OrderUpdate dataSyncLogOrderUpdate, DataSyncLogProcessorForSave dataSyncProcessorForSave,
        ...
        Logger logger, DbConnectionInfoProvider dbConnectionInfoProvider)
        {
            ...
            _dataSyncOrderUpdate = dataSyncLogOrderUpdate;
            ...
            _syncManager = new SyncManager(dataSyncLogOrderUpdate, _logger);
        ...
        private void AddErrorLogToListBox(string message)
        {
            if (lstSyncErrorLog.InvokeRequired)
            {
                lstSyncErrorLog.Invoke(new Action<string>(AddErrorLogToListBox), message);
            }
            else
            {
                lstSyncErrorLog.Items.Add(message);
            }
        }
        private async void  btn_order_sync_Click(object sender, EventArgs e)
        {
            _syncManager.SetDataSyncProcessor(_dataSyncOrderUpdate);
            // 동기화 작업 시작
            //UpdateStatus("동기화 작업 시작");
