...
namespace DioImplant_batch
{
    public class DataSyncLogProcessor
    {
        ...
        private const int BatchSize = 10000; // 배치로 처리할 로그 수
        ...
        public DataSyncLogProcessor(IHubContext<SyncStatusHub> hubContext)
        {
            InitMsDb();
        ...
        public async Task ProcessLogsAsync()
        {
            ...
            await _hubContext.Clients.All.SendAsync("ReceiveStatusUpdate", "Process Start-"+ logData.Rows.Count.ToString()+'건'+'-'+ DateTime.Now.ToString());
